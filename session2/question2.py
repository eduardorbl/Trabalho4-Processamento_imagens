...
def process_image(image_path, output_folder, scale=2.25, display=False):
    """
    Process a single image by resizing and applying bilinear interpolation.
    ...
    grid_output_path = os.path.join(output_folder, f"grid_bilinear_{image_name}")
...
if __name__ == "__main__":
    ...
    scale = 3.2
    display = True
    process_image(image_path=image_path, output_folder=output_folder, scale=scale, display=display)